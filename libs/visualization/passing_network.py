...
def get_config(config_team):
    config = read_json("visualization/plot_config.json")[config_team]
    height = float(config["height"])
    width = float(config["width"])
    return config,height,width


def _point_to_meters(p,config_team):
    '''
    Convert a point's coordinates from a 0-1 range to meters.
    '''
    config,height,width = get_config(config_team=config_team)
    return np.array([p[0]*width, p[1]*height])


def _meters_to_point(p,config_team):
    '''
...
def _change_range(value, old_range, new_range):
    ...
    '''
    line_width = ((value-old_range[0]) / (old_range[1]-old_range[0])) * (new_range[1]-new_range[0]) + new_range[0]
    return ((value-old_range[0]) / (old_range[1]-old_range[0])) * (new_range[1]-new_range[0]) + new_range[0]


def draw_pitch(min_x=0, max_x=1,config_team='home'):
    """
    ...
    # Plot outer lines
    line_pts = [
        [_point_to_meters([0, 0],config_team=config_team), _point_to_meters([0, 1],config_team=config_team)],  # left line
        [_point_to_meters([1, 0],config_team=config_team), _point_to_meters([1, 1],config_team=config_team)],  # right line
        [_point_to_meters([0, 1],config_team=config_team), _point_to_meters([1, 1],config_team=config_team)],  # top line
        [_point_to_meters([0, 0],config_team=config_team), _point_to_meters([1, 0],config_team=config_team)],  # bottom line
    ]
    ...
    # Plot boxes
    line_pts = [
        [_point_to_meters([0.5, 0],config_team=config_team), _point_to_meters([0.5, 1],config_team=config_team)],  # center line
...
def draw_pass_map(ax, player_position,
                  ...
                  max_player_count=None, max_player_value=None, max_pair_count=None, max_pair_value=None,debug=False,
                  config_team='home',team_players=None):
    """
    ...
    # Step 2: plot nodes
    # Combine num_passes and pass_value columns into one DataFrame
    team_players = [player.split(" ")[-1] for player in team_players]
    ...
    for player_name, row in player_stats.iterrows():
        ...
        if player_name in team_players:
            ...
            node_cmap = cm.get_cmap(config["nodes_cmap_sub"])
        ...
        if player_name in team_players:
            ...
            ax.annotate(player_name, xy=(player_x, player_y), ha="center", va="center", zorder=7,
                        fontsize=config["font_size"], color=config['font_color_sub'], weight='bold',
                        path_effects=[pe.withStroke(linewidth=2, foreground=background_color)])
